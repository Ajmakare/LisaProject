...
def all_programs(request):
    ...
    upjunctions = UPJunction.objects.filter(user=user, completed=False, start_date__gte=today).order_by('start_date')
...
def remove_video(request, program_id):
    if request.method == 'POST':
        program = get_object_or_404(Program, pk=program_id)
        video_id = request.POST.get('video_id')
        video = get_object_or_404(Video, pk=video_id)
        pv_junction = get_object_or_404(PVJunction, program=program, video=video)
        pv_junction.delete()
        messages.success(request, f"Successfully removed '{video.name}' from '{program.name}' program.")
